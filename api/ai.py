...
- Pound Fabrics / warehouse picklist (Shopify order export → picklist tabs). Title like "Pound Fabrics Picklist". Typical flow (do NOT use volume-balance GS-10 or XF-01-as-Flagged):
  1) GS-01 read the user's sheet (real url; optional sheet_name for the orders tab). Set read_order_range=true (tab "Picklist Run", start_cell B1, end_cell B2). Client fills those two cells with the start/end order numbers before each run; blank cells hard-fail. Returns start/end.
  2) XF-01: id_column=Name, min_id="{{{{step_1.output.start}}}}", max_id="{{{{step_1.output.end}}}}" (Shopify '#694358' is 694358). Keep every line of every order in that window.
  3) XF-02 group-aware drop: group_column=Name (order number), match_column=Lineitem name, op=contains, value="Express Shipping" — drop EVERY line of any order that has an Express Shipping line, not just that line.
  4) XF-05 aggregate: sku_column=Lineitem sku, qty_column=Lineitem quantity, min_count=4, format_string="{{qty}}m x {{count}}", write_summary_to_qty=true. LITERAL SOP: merge ONLY when the SAME SKU AND SAME quantity appear 4+ times. Never merge on SKU alone. Groups of 3 stay unmerged. Do not reverse-engineer a different collapse rule from a historical workbook.
  5) GS-10 emit with split_mode="sku_prefix_bands" (NOT target_rows_per_tab / keep_groups_intact): exception_field=Lineitem sku; sku_column=Lineitem sku; product_name_column=Lineitem name; sheet1_before_product_name="Plain Polycotton Fabric"; sku_prefix_breaks=["COT","DF","F","G","L","S"]; optional template_sheet_name="Picklist Template". Sheet 1 boundary is PRODUCT NAME (before Plain Polycotton Fabric), not SKU prefix. Sheets 2–8 are SKU-prefix bands (before COT / DF / F / G / L / S, then rest). Missing SKU → Exceptions tab.
  6) Optional GS-11 format on the emitted tabs (bold/borders/banding). Print margins come from the template tab duplicate, not GS-11.
- Prefer 2–6 steps (picklist may use up to 6). Be practical, not generic.
